...
# 测试 order-group，使用一个真实的 ord 请求路径示例:
...
# # 测试 user-group，使用一个真实的用户请求路径示例: (请取消注释并注释掉上面的 order-group 设置来测试 user-group)
...
if __name__ == "__main__":
    ...
    # 例如，如果后端服务 8081 返回文本 "Service A", 8083 返回 "Service B", 则设置为 {"Service A": 3, "Service B": 7}
    # 如果你不确定后端返回什么，可以先用浏览器或 curl 请求一次 Gateway 的测试路径 (例如 http://127.0.0.1:8080/ord/item/1)，看看响应内容是什么，然后用来更新 EXPECTED_WEIGHTS 的键。
