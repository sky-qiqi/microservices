...
// 这个配置类只为 "provider-test1" LoadBalancer 提供 Bean
@Configuration
public class ProviderTest1LoadBalancerConfiguration {

    @Bean
    public ServiceInstanceListSupplier providerTest1ServiceInstanceListSupplier() {
        List<ServiceInstance> instances = Arrays.asList(
                // ServiceInstance 的参数: instanceId, serviceId, host, port, isSecure
                // serviceId 必须匹配 LoadBalancerClient 的 name ("provider-test1")
                new StaticServiceInstance("provider-test1-instance-1", "provider-test1", "127.0.0.1", 8086, false)
        );

        return new ServiceInstanceListSupplier() {
            @Override
            public String getServiceId() {
                return "provider-test1"; // 必须匹配 @LoadBalancerClient 中的 name
            }
